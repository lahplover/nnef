...
"""
design multiple sequences for a sample of protein backbones using annealing. 
"""
...
protein_sample = pd.read_csv(f'{root_dir}/sample_train.csv')
...
for pdb_id in pdb_selected:
    ...
    for i in range(num_iter):
        ...
        annealer = AnnealSeq(energy_fn, protein, T_max=args.T_max, T_min=args.T_min, L=anneal_steps)
        annealer.run()
        ...
        with h5py.File(f'{root_dir}/{save_dir}/{pdb_id}_profile_{i}.h5', 'w') as f:
        ...
        pd.DataFrame({'sample_energy': sample_energy}).to_csv(f'{root_dir}/{save_dir}/{pdb_id}_energy_{i}.csv', index=False)
