...
_, pdb_id = os.path.split(args.input_pdb)
seq, coords_native, profile = load_protein_pdb(args.input_pdb, mode, device)
...
if fold_engine == 'anneal':
    ...
    if args.x_type == 'cart':
        minimizer = Dynamics(energy_fn, protein, num_steps=args.L, lr=args.lr, t_noise=args.T_max, log_interval=args.trj_log_interval)
    elif args.x_type == 'internal':
        minimizer = DynamicsInternal(energy_fn, protein, num_steps=args.L, lr=args.lr, t_noise=args.T_max, log_interval=args.trj_log_interval)
    elif args.x_type == 'int_fast':
        minimizer = DynamicsIntFast(energy_fn, protein, num_steps=args.L, lr=args.lr, t_noise=args.T_max, log_interval=args.trj_log_interval)
    elif args.x_type == 'mixed':
        minimizer = DynamicsMixed(energy_fn, protein, num_steps=args.L, lr=args.lr, t_noise=args.T_max, log_interval=args.trj_log_interval)
    elif args.x_type == 'mix_fast':
        minimizer = DynamicsMixFast(energy_fn, protein, num_steps=args.L, lr=args.lr, t_noise=args.T_max, log_interval=args.trj_log_interval)
    else:
        raise ValueError('x_type should be cart / internal / mixed / int_fast / mix_fast.')
...
write_pdb_sample2(seq, sample, pdb_id, 'sample', save_dir)
...
df.to_csv(f'{save_dir}/{pdb_id}_energy.csv', index=False)
